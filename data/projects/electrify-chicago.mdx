<ul className="flex flex-wrap gap-x-2 gap-4">
...
<article id="about-the-project">
  ...
  <div className="flex flex-wrap justify-center items-center gap-4 my-4">
    <BuildingCard
      ...
      gasUsage="0"
      emissions="4,309"
      path="https://electrifychicago.net/building/marina-towers-300-n-state-st/"
...
<article id="my-contributions">
  ...
  I created a <SectionLink text="city wide stats" link="https://github.com/vkoves/electrify-chicago/pull/192" /> 
  page, which gives a year over year analysis of the greenhouse gas emissions
  and other statistics for all of the buildings we have data for. I used Pandas
  to take our existing data and organize it by year, pass that data to Vue and
  render out the information with D3. Here are a couple of examples of the
  charts interactive charts I've created for it:
  <CityWideStats
    graphs={[
      {
...
<article id="next-steps">
  ...
  The current version only does all buildings but the next step is to make it so
  that it can be broken down by building type (mixed residential, healthcare,
  College, Data Center, etc). Once setting up the Pandas data pipeline, I sent
  that over to my graphs I made in D3 and rendered them to the screen!
</article>

<article id="technical-details">
  <Title title="Technical Details" />
  The frontend is built in VueJS, and uses a Jamstack framework called Gridsome.
  Data is collected from the <SectionLink
    text="Chicago Data Portal"
    link="https://data.cityofchicago.org/" /> and is sent through a Python / Pandas data pipeline.
</article>
